...
path_to_pr_id = main_folder + '\\Group Identifier.xlsx'
...
def add_info(df,path_to_identifier): 
    dfgroups = pd.ExcelFile(path_to_pr_id).parse() #FILE PATH!
    
    sexes = []
    groups = []
    for i in df['Subject']:
        sexes.append(dfgroups['Sex'][dfgroups['ID'] == i].values[-1].upper())
        groups.append(dfgroups['Group'][dfgroups['ID'] == i].values[-1].upper())
...
def individ_rr_graphs(df):
    df_indiv = df.explode('All ITRR (resp/min)')
    df_indiv['ITRR (resp/min)'] = df_indiv['All ITRR (resp/min)'].astype('float')
...
def summary_graphs(df):
    ...
    sns.barplot(x = 'Group', y = 'Correct Presses', data= df, ci = 68,ax= axes3[0,0], hue='Sex')
    sns.barplot(x = 'Group', y = 'Response Rate (resp/min)', data = df, ci = 68,  ax = axes3[0,1], hue='Sex')
    sns.barplot(x = 'Group', y = 'Incorrect Presses', data = df, ci=68, ax = axes3[1,0], hue='Sex')
    sns.barplot(x = 'Group', y = 'Correct Presses', data= df, ci = 68,  ax = axes3[1,1], hue='Sex')
...
def group_itrr_graph(df):
    ...
    plt.figure(figsize=(24,24))
    hue = df_indiv['Sex'].astype(str) + ', ' + df_indiv['Group'].astype(str)
    sns.lineplot(x = 'IDI', y = 'ITRR (resp/min)', data = df_indiv, hue = hue, err_style='band')
...
def parse():
    ...
    listoftraining = [str(i).upper() for i in msn_df['Training MSNS'].dropna()]
    listoftests = [str(i).upper() for i in msn_df['Test MSNS'].dropna()]
...
#     create_groups(food_df)
    add_info(food_df, path_to_pr_id)
    ...
    add_info(test_df, path_to_pr_id)
    ...
    with pd.ExcelWriter(path_to_save + f'PR Data from {date.today()}.xlsx') as writer:
        ...
        test_df.groupby(['Group', 'Sex', 'Day Number']).mean().to_excel(writer, sheet_name ='Group x Sex x Day Means')
        test_df.groupby(['Group', 'Sex', 'Day Number']).sem().to_excel(writer, sheet_name ='Group x Sex x Day SEMs')
...
def parse_and_graph():
    ...
    msn_df = pd.ExcelFile(path_to_pr_id).parse()
    listoftraining = [str(i).upper() for i in msn_df['Training MSNS'].dropna()]
    listoftests = [str(i).upper() for i in msn_df['Test MSNS'].dropna()]
