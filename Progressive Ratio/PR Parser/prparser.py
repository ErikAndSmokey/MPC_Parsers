...
def parse():
    ...
    path_to_last = main_folder + '\\Last Three\\'
    list_o_subs = []
    for i in os.listdir(path_to_data):
        file = path_to_data + i
        list_o_subs.append(MainInfoParser.subject_grabber(file))
    list_o_subs = list(set(list_o_subs))

    for i in list_o_subs:
        track_dates = []
        for x in os.listdir(path_to_data):
            if i in x:
                track_dates.append(MainInfoParser.date_grabber(path_to_data + x))
        track_dates = [pd.to_datetime(i) for i in track_dates]
        track_dates.sort()
        track_dates = [i.date() for i in track_dates[-3:]]
        for a in track_dates:
            for b in os.listdir(path_to_data):
                if i in b and str(a) in b:
                        copyfile(path_to_data+b, path_to_last+b)
                else:
                    pass
    ...
    for i in os.listdir(path_to_last):
        print(i)
        file = path_to_last + i
